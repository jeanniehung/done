...
def change(array):
    ...
    with open('文件.conf', 'r') as read_f,\
        ...
        tag1 = True
        for item in read_f:
            if item.strip() == backend_data:    # 找到文件  backend www.hjc1.org 这一行
                tag = True                      # 改变tag的状态
                continue                        # 继续读下一行

            if tag:                             # 修改的数据在 backend www.hjc1.org 的下一行可以开始操作
                if tag1:
                    res.insert(0, '%s\n' % backend_data)  # res里面没有 backend www.hjc1.org ，自己加上去，
            ...
            if not tag:     # 没有找到 backend www.hjc1.org 就把文件的内容一行一行写到新文件中
                            # 放在第一个位置，文件修改内容在第一个目录下，第一个目录名会多一个；
