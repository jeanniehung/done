...
def com_mul(start, end, num1, num2, count=0):
...
可变长参数：*args ：只接收位置参数，或者列表，元祖
          **kwargs ：只接收关键字参数，和字典格式
"""
...
def func_sort(wait_l, args):
    for i in range(len(wait_l)):
        if wait_l[i] > wait_l[i-1]:
            temp = wait_l[i]
    args.append(temp)
    func_sort(wait_l.pop(min(wait_l)), args)
    return args
